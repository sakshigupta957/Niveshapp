...
function GroupMembers() {
  ...
  const mkadm = async e=>{
    try{
      ...
      const responseData = await sendRequest(
          `https://stool-backend.vercel.app/api/groups/transferownership/removegroup/${userId}/`,"POST",body,{
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const responseData = await sendRequest(
          `https://stool-backend.vercel.app/api/groups/groupmembers/${gid}`,"POST"
        );
